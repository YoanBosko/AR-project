...
public sealed class SegmentationFilter : System.IDisposable
{
    ...
    #if PLATFORM_ANDROID
        const int Width = 320 + 1;  // Lebih kecil untuk mobile
        const int Height = 176 + 1;
    #else
    ...
    public void ProcessImage(Texture sourceTexture)
    {
        ...
        // Debug output tensor
        Debug.Log($"Input Tensor Shape: (1, {Height}, {Width}, 3)");
        Debug.Log($"Output Tensor Shape: {output.shape}");
        // Debug.Log($"Output Tensor Min: {output.Min()}");
        // Debug.Log($"Output Tensor Max: {output.Max()}");
